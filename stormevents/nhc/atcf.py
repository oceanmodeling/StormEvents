...
def atcf_url(
    ...
    if mode is None:
        ...
        entry = get_atcf_entry(
            basin=nhc_code[:2], storm_number=int(nhc_code[2:4]), year=year
        )
...
def read_atcf(
    ...
    data.astype(
        {field: "string" for field in data.columns},
        copy=False,
    )
    ...
    if "USERDEFINED" in data and data["USERDEFINED"].str.contains(",").any():
        ...
        try:
            lines = (
                str(line, "UTF-8") if isinstance(line, bytes) else line for line in atcf
            )
            lines = (
                (
                    entry.strip()
            ...
            extra_data = DataFrame.from_records(
                lines,
                columns=list(extra_fields),
            ).astype({field: "string" for field in extra_fields})
    ...
    data.loc[best_track_records, "YYYYMMDDHH"] += data.loc[
        best_track_records, "TECHNUM/MIN"
    ]
    ...
    data.loc[data["LatN/S"].str.endswith("S"), "LatN/S"] = "-" + data[
        "LatN/S"
    ].str.strip("S")

    data.loc[data["LonE/W"].str.endswith("E"), "LonE/W"] = data["LonE/W"].str.strip("E")
    data.loc[data["LonE/W"].str.endswith("W"), "LonE/W"] = "-" + data[
        "LonE/W"
    ].str.strip("W")

    data[["LatN/S", "LonE/W"]] = (
        data[["LatN/S", "LonE/W"]].astype(
            {"LatN/S": float, "LonE/W": float}, copy=False
        )
        / 10
    )
    ...
    for float_field in float_fields:
        data.loc[
            (data[float_field].str.len() == 0) | pandas.isna(data[float_field]),
            float_field,
        ] = "NaN"
    ...
    data.rename(
        columns={
            **{
                field: value
                for field, value in ATCF_FIELDS.items()
                if field in data.columns
            },
            **{
                field: value
