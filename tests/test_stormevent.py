...
def test_time_interval():
    ...
    ida2021 = StormEvent(
            'ida', 2021, start_date=datetime(2021, 8, 30),
    ...
    assert (
            repr(
                florence2018) == "StormEvent('FLORENCE', 2018, start_date='2018-09-16 12:00:00')"
    )

    assert paine2016.start_date == datetime(2016, 9, 18)
    assert paine2016.end_date == datetime(2016, 9, 19)
    assert repr(
        paine2016) == "StormEvent('PAINE', 2016, end_date='2016-09-19 00:00:00')"
...
def test_track(florence2018, ida2021):
    ...
    ida_reference_track = VortexTrack.from_storm_name(
            ida2021.name, ida2021.year, start_date=ida2021.start_date,
            end_date=ida2021.end_date
    )

    pandas.testing.assert_frame_equal(florence_track.data,
                                      florence_reference_track.data)
...
def test_high_water_marks(florence2018):
    ...
    pandas.testing.assert_frame_equal(high_water_marks,
                                      reference_high_water_marks)
...
def test_tidal_data_within_isotach(florence2018):
    ...
    tidal_data = florence2018.tidal_data_within_isotach(
            wind_speed=34, start_date=datetime(2018, 9, 13, 23),
...
def test_tidal_data_within_region(florence2018):
    null_track = florence2018.track(
        end_date=florence2018.start_date + timedelta(minutes=1))
    null_data = florence2018.tidal_data_within_region(
            region=box(*null_track.linestring.bounds),
